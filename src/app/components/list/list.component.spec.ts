...
describe('ListComponent', () => {
  ...
  beforeEach(
    waitForAsync(() => {
      TestBed.configureTestingModule({
        imports: [CommonModule, BrowserAnimationsModule, ListModule],
      }).compileComponents();
    })
  );
  ...
  it('should display', () => {
    ...
    fixture.detectChanges();
    const queryHeader = Array.from(
      fixture.debugElement.nativeElement.querySelectorAll('th')
    ).map((e: any) => e.innerText.toLowerCase());
    const queryRow = Array.from(
      fixture.debugElement.nativeElement.querySelectorAll('td')
    ).map((e: any) => e.innerText.toLowerCase());
    const hasAllHeaders = Object.keys(mockData).reduce(
      (result: boolean, key: string) =>
        result && queryHeader.includes(key.toLowerCase()),
      true
    );
    const hasAllRows = Object.values(mockData)
      ...
      .reduce(
        (result: boolean, key: string) =>
          result && queryRow.includes(key.toLowerCase()),
        true
      );
